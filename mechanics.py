...
OPPOSITE_EDGES, EDGES, COLORED_PIECES,PAWN_DOUBLES,MOVEMENTS,STARTING_POSITIONS,ERROR_MESSAGE,COLORS,FORMAL,LETTERS,PIECE_TYPES,FORMAL_COLORS,INIT_COLOR,QSIDE_KSIDE,START_BOARD,INDEX_MOVES,WHITE_PIECES,BLACK_PIECES = var.oppositeEdges, var.edges, var.coloredPieces, var.dblMoves, var.movements, var.startingPositions, var.error_messages, var.colors, var.formals, var.letters, var.piece_types, var.formalColors, var.initial_color, var.startingSideIndices, var.starting_bits, var.indexMoves, var.white_pieces, var.black_pieces
...
def bit_to_indices(piece, board):
    ...
    for i in range(64):
        if board[piece][i] == "1":
            indices.append(i)
    return indices
def moves_postBlock(piece, board, dblMoveIndex, active, inactive, blockSame = True):
    """
        ...
        if piece in['p', 'P']:
            ...
            if dblMoveIndex != None:
                if position not in edges:
                    if (position + 1 == dblMoveIndex) or (
                        position - 1 == dblMoveIndex
                    ):
                        if piece == "P":
                            captures.append(
                                (position, dblMoveIndex + 8, dblMoveIndex)
                            )
                        if piece == "p":
                            captures.append(
                                (position, dblMoveIndex - 8, dblMoveIndex)
            ...
            for cap in [cap1, cap2]:
                ...
                if p in inactive:
                    ...
                    protections.append((position, cap1))
        else:
...
def isThreatened(piece, board, dblMoveIndex, active, inactive):
    threats = []
    for opp in inactive:
        captures = moves_postBlock(opp, board, dblMoveIndex, inactive, active)[1]
        if len(captures) > 0:
            for cap in captures:
...
def isProtected(piece, board, dblMoveIndex, active, inactive):
    protections = []
    for ally in active:
        protects = moves_postBlock(ally, board, dblMoveIndex, inactive, active)[1]
        if len(protects) > 0:
...
def boardAfterMove(board, piece, move, inactive):
        ...
        return newBoard
def postCheckMoves(color, piece, board, dblMoveIndex, active, inactive):
    legal1, capture1 = moves_postBlock(piece, board, dblMoveIndex, active, inactive)[:2]
    legal2, capture2 = [], []
    for move in legal1:
...
class Board:
    ...
    def allMoves(self):
        ...
        for piece in self.active:
            moves = postCheckMoves(self.c, piece, self.board, self.dblMoveIndex, self.active, self.inactive )
            for i in range(2):
                for m in moves[i]:
    ...
    def affectMove(self):
        ...
        if move == "O-O":
            ...
            for section in allMoves:
                for mv in section:
                    if mv == move:
                        self.board = boardAfterMove(self.board, move[0], move[1], self.inactive)
        ...
        if not self.running:
            if not self.Break:
                ...
                if self.stalemate:
                    ...
                    print(f"Game Over. {FORMAL_COLORS[self.alternateColor()]} Wins by Checkmate!")
