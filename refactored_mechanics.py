...
class Board:
    def __init__(self):
        ...
        self.kingHasMoved = {"w": False, "b": False}
    ...
    def afterBoard(self):
        if self.running:
            if not self.Break:
                ...
                print(f"Game Over. {FORMAL_COLORS[c]} Wins by Checkmate!")
                playagain = input(
                    "Would you like to play again? Y [Play Again] | Any-Other-Key [Exit] \n"
                )
                if playagain == "Y":
        ...
        for piece in self.board:
            if self.c == "w":
                ...
                if piece in BLACK_PIECES:
                    ...
                    inactive[piece] = self.board[piece]
        self.active, self.inactive = active, inactive

    def castleAvailable(self):
        pass
    ...
    def postCheckMoves(self, piece):
        ...
        for move in legal1:
            ...
            check = isCheck(self.active, newboard, self.dblMoveIndex)
            if not check:
                legal2.append(move)
        for move in capture1:
            newboard = self.boardAfterMove(piece, move)
    ...
    def moves_postBlock(self, piece):
        """
        returns two lists of tuples, where the tuples are (old_position, new_position).
        1st list: non-capture moves
        2nd list: capture moves
        ...
        for position in all_positions:
            if piece == self.pawn:
                ...
                if pieceInIndex(self.board, fwd[0]) == None:
                    if dbl:
                        if pieceInIndex(self.board, fwd[1]) == None:
                            legal.append((position, fwd[1]))
                    legal.append((position, fwd[0]))
                if pieceInIndex(self.board, cap1) in self.inactive:
                    captures.append((position, cap1))
                if pieceInIndex(self.board, cap2) in self.inactive:
                    captures.append((position, cap2))
            else:
                for move_sect in self.movements[piece]:
                    ...
                    for i, move in enumerate(move_sect):
                        newPos = get_newPos(position, move)
                        if onBoard(newPos):
                            squareCheck = self.pieceInIndex(newPos)
                            if squareCheck != None:
    ...
    def isThreatened(self, piece):
        threats = []
        for opp in self.inactive:
            captures = self.moves_postBlock(opp, active=False)[1]
            caps = len(captures)
            if caps > 0:
                for i in range(caps):
    ...
    def castlePossible(self):
        qside, kside = True, True
        if isCheck(self.active, self.board, self.dblMoveIndex):
            return False
        if c == "w":
            k, r = "K", "R"
        else:
